...
import { añadirMallaCurricular } from "./planeacion-docentes-modules/funcionalidades/datatable-docentes-disponibles/añadir-malla-curricular.js";
import { añadirProgramaDocente } from "./planeacion-docentes-modules/funcionalidades/modal-crear-docente-estandar/añadir-programas-docente.js";
import { removerProgramaModalCrearDocente } from "./planeacion-docentes-modules/funcionalidades/modal-crear-docente-estandar/remover-programa-modal.js";
...
import { alertaPreload } from "./planeacion-docentes-modules/utils/alertas.js";
...
/** Filtros facultades - programas */
$("body").on(
    "change",
    '#facultades input[type="checkbox"], #todosFacultad',
    async function () {
        await estadoFiltros.actualizarEstado();
        actualizarProgramas(
            estadoFiltros.obtenerFiltros(),
            urlActualizarFiltros
        );
    }
);
...
/** Agregar malla al select de asignaturas disponibles */
$(document).on("change", ".select-añadir-programa", async function () {
    await añadirMallaCurricular($(this), urlTraerMallaPrograma);
});

/*----- Funcionalidades Modal crear docente -----*/

/** Modal crear docente */
$(document).on("click", "#button-crear-docente", async function (event) {
    event.preventDefault();
    await cargarProgramasModal(urlFiltrosProgramasFacultad);
    $("#modal-nuevo-docente").modal("show");
});

/** Añadir programas para crear docente */
$(document).on("change", "#programas-docente", async function () {
    await añadirProgramaDocente($(this));
});

/** Remover programa */
$(document).on(
    "click",
    ".button-remover-programa-modal-crear",
    async function () {
        await removerProgramaModalCrearDocente($(this));
    }
);

/** Añadir asignatura */
$(document).on("change", "#asignaturas-docente", async function () {
    await añadirAsignaturaModalDocente($(this));
});
...
$(document).on(
    ...
    async function () {
        await removerAsignaturaModal($(this));
    }
);
...
const dataTableDocentes = async (disponibilidad = "activos") => {
    ...
    let programasSelect = estadoFiltros.obtenerProgramas();

    let response = await enviarAjax(data, urlDocentesDisponibles);

    if (response.length > 0) {
        Swal.close();
        /** Añadir los programas de los filtros a la data */
        let combinedData = response.map((data) => {
            return {
                nombre: data.nombre,
                email: data.email,
                id_banner: data.id_banner,
                codigos_materia: data.codigos_materia,
                codigos_programa_materia: data.codigos_programa_materia,
                cupo: data.cupo,
                cupo_16_semanas: data.cupo_16_semanas,
                malla: data.malla,
                programas_docente: data.programas_docente,
                disponibilidad: data.disponibilidad,
                programasSelect: programasSelect,
            };
        });

        let headers = await headerDataTableDocentesDisponibles(combinedData);
...
async function cargarDataConPreload() {
    ...
    try {
        // Llamada a las funciones que cargan las tablas de datos
...
$("#todosFacultad").on("click", function (event) {
    event.stopPropagation();
});

$("#todos-programa").on("click", function (event) {
    event.stopPropagation();
});

$("#todosFacultad").change(function () {
    $("#facultades input[type='checkbox']").prop(
        "checked",
        $(this).is(":checked")
    );
});

$("#todos-programa").change(function () {
    $("#programas input[type='checkbox']").prop(
        "checked",
        $(this).is(":checked")
    );
});
